...
if not opt.testing:
    ...
    slu_optimizer, slu_scheduler = set_optimizer(slu_model, lr=slu_params['lr'], l2=slu_params['l2'],
    ...
    nlg_optimizer, nlg_scheduler = set_optimizer(nlg_model, lr=nlg_params['lr'], l2=nlg_params['l2'], max_norm=nlg_params['max_norm'], lr_schedule='constant')
    ...
    for i in range(opt.max_epoch):
        ...
        slu_pseudo_dataset = slu_pseudo_labeling(slu_model, slu_vocab, slu_evaluator, unlabeled_dataset, opt.test_batchSize, device=slu_device, beam=opt.beam) \
        ...
        nlg_pseudo_dataset = nlg_pseudo_labeling(nlg_model, nlg_vocab, nlg_evaluator, unlabeled_dataset, opt.test_batchSize, device=nlg_device, beam=opt.beam) \
        ...
        for j in range(0, nsentences, opt.batchSize):
            ...
            if 'slu' in opt.cycle_choice and len(slu_pseudo_dataset) != 0:
                ...
                slot_scores, intent_scores = slu_model(inputs, lens, outputs)
                ...
                surface_scores, slot_hist = nlg_model(intents, slots, slot_lens, lens, outputs[:, :-1], slot_states, copy_tokens)
            ...
            slu_model.pad_embedding_grad_zero()
            slu_optimizer.step()
            slu_scheduler.step()
            nlg_model.pad_embedding_grad_zero()
            nlg_optimizer.step()
            nlg_scheduler.step()
        ...
        dev_slot, dev_intent = slu_decode(slu_model, slu_vocab, slu_evaluator, dev_dataset, os.path.join(exp_path, 'slu_valid.iter' + str(i)),
        ...
        start_time = time.time()
        test_slot, test_intent = slu_decode(slu_model, slu_vocab, slu_evaluator, test_dataset, os.path.join(exp_path, 'slu_test.iter' + str(i)),
            opt.test_batchSize, device=slu_device, beam=opt.beam, n_best=opt.n_best)
        ...
        if slu_coefficient * dev_slot + (1 - slu_coefficient) * dev_intent > prev_best:
            torch.save(slu_model.state_dict(), open(os.path.join(exp_path, 'slu_model.pkl'), 'wb'))
        ...
        ##### Evaluate on dev and test dataset #####
        start_time = time.time()
        dev_bleu, dev_slot = nlg_decode(nlg_model, nlg_vocab, nlg_evaluator, dev_dataset, os.path.join(exp_path, 'nlg_valid.iter' + str(i)),
        ...
        start_time = time.time()
        test_bleu, test_slot = nlg_decode(nlg_model, nlg_vocab, nlg_evaluator, test_dataset, os.path.join(exp_path, 'nlg_test.iter' + str(i)),
            opt.test_batchSize, device=nlg_device, beam=opt.beam, n_best=opt.n_best)
        ...
        if nlg_coefficient * dev_bleu + (1 - nlg_coefficient) * dev_slot > prev_best:
            torch.save(nlg_model.state_dict(), open(os.path.join(exp_path, 'nlg_model.pkl'), 'wb'))
