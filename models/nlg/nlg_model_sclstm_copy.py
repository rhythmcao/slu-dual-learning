...
class SCLSTMCopyModel(NLGModel):
    ...
    def forward(self, intents, slots, slot_lens, lens, dec_inputs, slot_states, copy_tokens):
        ...
        feats, _, slots, pointer, gates = self.decoder(inputs, intents, memory, decoder_state, slot_states, slot_lens, all_slot_hist=True)
        log_prob = self.generator(feats, pointer, gates, copy_tokens)
    ...
    def decode_one_step(self, ys, intents, memory, dec_states, slot_states, slot_lens, copy_tokens):
        ...
        feats, dec_states, slot_states, pointer, gates = self.decoder(inputs, intents, memory, dec_states, slot_states, slot_lens)
        logprob = self.generator(feats, pointer, gates, copy_tokens).squeeze(dim=1) # bsize x vocab_size
